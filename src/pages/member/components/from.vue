        ...
        <div class="block-item">
          ...
          <div class="select-group">
            <select class="js-province-selector" v-model="provinceValue">
              ...
              <option :value="p.value" v-for="(p,index) in addressData.list" :key=index>{{p.label}}</option>
            </select>
            <select class="js-city-selector" v-model="cityValue">
              <option value="-1">选择城市</option>
              <option :value="c.value" v-for="(c,index) in cityLists" :key=index>{{c.label}}</option>
            </select>
            <select class="js-county-selector" name="area_code" v-model="districtValue">
              <option value="-1">选择地区</option>
              <option :value="d.value" v-for="(d,index) in districtLists" :key=index>{{d.label}}</option>
            </select>
          </div>
        </div>
        <div class="block-item">
